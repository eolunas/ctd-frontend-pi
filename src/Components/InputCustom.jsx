    ...
    <>
      <div className='relative w-full flex justify-center'>
        <input
          ...
          onBlur={() => setTimeout(() => setIsFocused(false), 350)}
          type='text'
        ...
        {value && (
          <a
            className='absolute w-9 h-full flex text-black  justify-center items-center right-0 rounded-md cursor-pointer'
            onClick={cleanField}
          >
            <svg
              height='20'
        ...
        {value && isFocused && (
          <div className='absolute left-0 top-11 w-full text-black rounded-md shadow-md border p-2 bg-white text-sm'>
            {data
              .filter((item) => {
              ...
              .map((item, index) => (
                <div
                  key={`${index}${item}`}
                  className='flex cursor-pointer m-1 hover:bg-slate-400'
