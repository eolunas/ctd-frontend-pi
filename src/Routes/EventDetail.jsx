...
const EventDetail = () => {
  const { id } = useParams();
  const { state, toggleFavorite } = useCharStates(); // Contexto con toggleFavorite
  ...
  const [isFavorite, setIsFavorite] = useState(false); // Control del estado favorito
  const isFav = state.favs.some((item) => item.id === event.id);
  ...
  useEffect(() => {
    const getEventById = async () => {
      try {
        ...
        const isFav = state.favs.some((fav) => fav.id === Number(id));
        setIsFavorite(isFav);
      } catch (error) {
        console.error(error);
        setIsErrorOpen(true);
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      await toggleFavorite(state.user.id, event.id); // Alternar favorito
      setIsFavorite(!isFavorite); // Actualizar estado local
    } catch (error) {
  ...
  return (
    <>
      ...
      {isErrorModalDateOpen && (
        <ErrorMessage
          title="Lo sentimos :("
          description="Debes elegir una fecha para reservar."
          buttonText="Volver a Inicio"
